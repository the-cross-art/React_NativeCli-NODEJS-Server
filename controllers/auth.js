...
exports.register = async (req, res) => {
  try {
    const existUser = await User.findOne({ $or: [{ email: req.body.values.email }, { username: req.body.values.username }, { phone: req.body.values.phone }] })
    ...
    if (error.code === 11000)
      ...
      res.status(404).json("Please fill all the manditory fields");
  }
};

exports.login = async (req, res) => {
  try {
    const existUser = await User.findOne({ $or: [{ email: req.body.value }, { username: req.body.value }, { phone: req.body.value }] })
